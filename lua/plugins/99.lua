return {
	...
	config = function()
		...
		local CursorAgentForceProvider = setmetatable({}, { __index = _99.Providers.CursorAgentProvider })
		local CodexProvider = setmetatable({}, { __index = _99.Providers.OpenCodeProvider })

		-- 99 expects the agent to read/write temp files non-interactively.
		-- Cursor needs auto-approval for that path to work reliably.
		function CursorAgentForceProvider._build_command(_, query, context)
			return {
				"cursor-agent",
				"-f",
				"--model",
		...
		function CursorAgentForceProvider._get_provider_name()
		...
		function CodexProvider.fetch_models(callback)
			callback({ "gpt-5", "gpt-5-mini", "o4-mini" }, nil)
		end

		_99.Providers.CursorAgentProvider = CursorAgentForceProvider
		...
		_99.setup({
			provider = CursorAgentForceProvider, -- default: OpenCodeProvider
			logger = {
				level = _99.DEBUG,
		...
		vim.keymap.set("n", "<leader>9c", function()
			...
			vim.notify("99 provider: Cursor", vim.log.levels.INFO)
		end, { desc = "99: Use Cursor provider" })

		vim.keymap.set("n", "<leader>9o", function()
			_99.set_provider(_99.Providers.CodexProvider)
			vim.notify("99 provider: Codex", vim.log.levels.INFO)
		end, { desc = "99: Use Codex provider" })
	end,
}
